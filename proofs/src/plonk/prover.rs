        ...
        + FromUniformBytes<64>,
{
    ...
    // Upstream renamed compute_h_poly -> compute_nu_poly. The phase name is
    // kept as-is: downstream consumers match on these strings.
    checkpoint(ctx, "finalise.compute_h_poly.start")?;
    let nu_poly = compute_nu_poly(pk, &trace, &ctx.config);
...
pub(super) fn compute_nu_poly<F: WithSmallOrderMulGroup<3>, CS: PolynomialCommitmentScheme<F>>(
    ...
    // numerator nu(X) of h(X) = nu(X)/(X^n-1). The local keeps the patch's
    // name because it is internal and the spill scaffolding above refers to it.
